...
from app.models import User, Unit, Lease
...
def test_rooms_import_idempotent():
    ...
    from app.models import Company, Community, Building, Unit, Tenant, Lease, Bill, BillLine
    from sqlalchemy import delete
    with Session(engine) as s:
        comp = s.exec(select(Company).where(Company.code == "TESTR")).first()
        if comp:
            # delete dependent units and buildings
            comms = s.exec(select(Community).where(Community.company_id == comp.id)).all()
            for comm in comms:
                blds = s.exec(select(Building).where(Building.community_id == comm.id)).all()
                for bld in blds:
                    units = s.exec(select(Unit).where(Unit.building_id == bld.id)).all()
                    for u in units:
                        s.exec(delete(BillLine).where(BillLine.bill_id.in_(select(Bill.id).where(Bill.unit_id == u.id))))
    ...
    res = client.post("/api/v1/imports/rooms", files=files, headers={"Authorization": f"Bearer {token}"})
    ...
    if body.get("created") is None and body.get("batch_id"):
        ...
        import time
        for _ in range(20):
            r2 = client.get(f"/api/v1/imports/batches/{batch_id}", headers={"Authorization": f"Bearer {token}"})
    ...
    res2 = client.post("/api/v1/imports/rooms", files=files, headers={"Authorization": f"Bearer {token}"})
    ...
    if body2.get("created") is None and body2.get("batch_id"):
        ...
        import time
        for _ in range(20):
            r3 = client.get(f"/api/v1/imports/batches/{batch_id}", headers={"Authorization": f"Bearer {token}"})
...
def test_leases_import_rollback_on_overlap():
    ...
    r = client.post("/api/v1/imports/rooms", files=files_rooms, headers={"Authorization": f"Bearer {token}"})
    ...
    with Session(engine) as s:
        ...
        if not existing:
            ...
            s.add(t)
            s.flush()
            lease_rec = Lease(unit_id=uid, tenant_id=t.id, start_date=date(2026, 2, 1), end_date=date(2026, 2, 28), rent_amount=1000, deposit_amount=1000)
            s.add(lease_rec)
            s.commit()
    ...
    res = client.post("/api/v1/imports/leases", files=files, headers={"Authorization": f"Bearer {token}"})
    assert res.status_code == 200
    body = res.json()
