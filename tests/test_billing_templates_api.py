...
def test_admin_crud_and_rbac():
    ...
    payload = {
        ...
        "items": [
            {
                "charge_item_id": ci1.id,
                "is_required": True,
                "sort_order": 2,
                "note": "n1",
            },
            {
                "charge_item_id": ci2.id,
                "is_required": False,
                "sort_order": 1,
                "note": "n2",
            },
    ...
    # retrieve
    r2 = client.get(
        f"/api/v1/templates/{tid}", headers={"Authorization": f"Bearer {admin_tok}"}
    )
    ...
    r_clerk_create = client.post(
        "/api/v1/templates/",
        json=payload,
        headers={"Authorization": f"Bearer {clerk_tok}"},
    )
    assert r_clerk_create.status_code == 403
    r_sales_create = client.post(
        "/api/v1/templates/",
        json=payload,
...
def test_list_and_instantiate_by_clerk_and_sales():
    ...
    with Session(engine) as session:
        ...
        tmpl = BillTemplate(
            name="TempInst", description="for inst", is_active=True, created_by=1
        )
        session.add(tmpl)
        session.commit()
        session.refresh(tmpl)
        l1 = BillTemplateLine(
            template_id=tmpl.id, charge_item_id=ci1.id, is_required=True, sort_order=1
        )
        l2 = BillTemplateLine(
            template_id=tmpl.id, charge_item_id=ci2.id, is_required=False, sort_order=2
        )
        session.add(l1)
        session.add(l2)
        session.commit()
    ...
    r_list_c = client.get(
        "/api/v1/templates/", headers={"Authorization": f"Bearer {clerk_tok}"}
    )
    assert r_list_c.status_code == 200
    r_list_s = client.get(
        "/api/v1/templates/", headers={"Authorization": f"Bearer {sales_tok}"}
    )
