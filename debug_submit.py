...
with Session(engine) as s:
    ...
    if not u:
        s.add(User(username='clerk1', password_hash=get_password_hash('cpass'), role='clerk'))
        s.commit()
...
res = client.post('/api/v1/bills/generate', params={'unit_id':uid,'date':'2026-02-15'}, headers={'Authorization':f'Bearer {token}'})
print('generate', res.status_code, res.text)
if res.status_code==200:
    bid = res.json()['bill_id']
    res2 = client.post(f'/api/v1/bills/{bid}/submit', headers={'Authorization':f'Bearer {token}'})
