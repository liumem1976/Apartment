...
def generate_bill_for_unit(unit_id: int, target_date: date, actor_id: Optional[int] = None) -> Bill:
    with Session(engine) as session:
        ...
        from .models import Unit, Building, Community, Company
        ...
        # audit
        audit = AuditLog(actor_id=actor_id, action="create_bill", before=None, after=f"bill:{bill.id}")
        session.add(audit)
...
def generate_batch_for_company(company_id: int, target_date: date, actor_id: Optional[int] = None) -> List[Bill]:
    bills = []
    with Session(engine) as session:
        leases = session.exec(select(Lease)).all()
        for lease in leases:
            try:
                bill = generate_bill_for_unit(lease.unit_id, target_date, actor_id=actor_id)
                bills.append(bill)
            except Exception:
                session.rollback()
                raise
